...
C Cherenkov exit foil
	...
	parameter (hcer_exit_radlen = 8.897)
	parameter (hcer_exit_thick  = 0.0125)


C The pre-Shower layer is made of TF-1 type lead glass. The radiation length
C for this material is 2.5-2.8cm. The shower layer is SF-5 type lead glass, with
C a radiation length of 2.55cm. So, I use 2.6cm for both layers.
	real*8 hcal_radlen
	parameter (hcal_radlen = 2.6)

C There is 13mm of Al before the pre-shower and 19mm of Al between the layers
	real*8 hcal_al_radlen,hcal_al_thicka,hcal_al_thickb
	parameter (hcal_al_radlen = 8.897)
	parameter (hcal_al_thicka  = 1.3)
	parameter (hcal_al_thickb  = 1.9)
...
C Straw Chamber position
	...
	parameter (hsc_zpos = 152.4) !entrance to chamber; remember z=0 is vdc 1 center
	parameter (hsc_left = 30.0)
	parameter (hsc_right = -30.0)
	parameter (hsc_y_offset = -5.48)
	parameter (hsc_top = -104.5)
	parameter (hsc_bottom = 104.5)
	parameter (hsc_x_offset = -13.9)
...
C Scintillator positions and thicknesses
	...
	parameter (hscin2_x_offset = -16.6)
...
C Cherenkov position
	...
	parameter (hcer_entr_z =  178.2)
	parameter (hcer_thick =  137.0)
	parameter (hcer_left =  30.2)
	parameter (hcer_right = -30.2)
	parameter (hcer_y_offset = -0.28)
	parameter (hcer_top = -122.3)
	parameter (hcer_bottom = 122.3)
	parameter (hcer_x_offset = 0.0)

C Calorimeter position

	real*8 hcal_ps_zpos,hcal_ps_left,hcal_ps_right,hcal_ps_top,hcal_ps_bottom
	real*8 hcal_ps_thick,hcal_ps_y_offset,hcal_ps_x_offset
	
	real*8 hcal_sh_left,hcal_sh_right,hcal_sh_top,hcal_sh_bottom
	real*8 hcal_sh_thick,hcal_sh_y_offset,hcal_sh_x_offset

	parameter (hcal_ps_zpos = 348.2) !may not be exactly mid-point
	parameter (hcal_ps_thick = 10.1)
	parameter (hcal_ps_left = 35.0)
	parameter (hcal_ps_right = -35.0)
	parameter (hcal_ps_y_offset = 0.0)
	parameter (hcal_ps_top = -121.2)
	parameter (hcal_ps_bottom = 121.2)
	parameter (hcal_ps_x_offset = -7.0)

	!parameter (hcal_sh_zpos = 363.2) !Not used right now
	parameter (hcal_sh_thick = 35.0)
	parameter (hcal_sh_left = 38.13)
	parameter (hcal_sh_right = -38.13)
	parameter (hcal_sh_y_offset = -0.38)
	parameter (hcal_sh_top = -114.4)
	parameter (hcal_sh_bottom = 114.4)
	parameter (hcal_sh_x_offset = -2.7)
...
C finished VDCs, drift to straw chamber
	drift = hsc_zpos - hdc_2_zpos - (hdc_thick*sqrt(2.))
	radw = drift/hair_radlen
	call project(xs,ys,drift,decay_flag,dflag,m2,p,pathlen)
	if(ms_flag) call musc_ext(m2,p,radw,drift,dydzs,dxdzs,ys,xs)
...
C Drift to front of Calorimeter.
	drift = hcal_ps_zpos - (hcal_ps_thick/2.) - hcal_al_thicka -  hscin2_zpos - (hscin2_thick/2.)
	radw = drift/hair_radlen
	call project(xs,ys,drift,decay_flag,dflag,m2,p,pathlen)
	if(ms_flag) call musc_ext(m2,p,radw,drift,dydzs,dxdzs,ys,xs)

	! Aluminum before Pre-Shower
	drift = hcal_al_thicka
...
C Check particle at back of Pre-Shower
C Right now particle must be at least 2.5cm from any edge.
*       Fix Me!!! Maybe should protect with a pid flag...
*       -------
	if (xs.gt.(hcal_ps_bottom+hcal_ps_x_offset-2.5) .or.
     >      xs.lt.(hcal_ps_top+hcal_ps_x_offset+2.5) .or.
     >      ys.gt.(hcal_ps_left+hcal_ps_y_offset-2.5) .or.
     >      ys.lt.(hcal_ps_right+hcal_ps_y_offset+2.5) ) then
	   rSTOP_ps = rSTOP_ps + 1
	   stop_where=30.
	   x_stop=xs
	   y_stop=ys
	...
	!Aluminum between layers
	drift = hcal_al_thickb
	radw = drift/hcal_al_radlen
	call project(xs,ys,drift,decay_flag,dflag,m2,p,pathlen)
	if(ms_flag) call musc_ext(m2,p,radw,drift,dydzs,dxdzs,ys,xs)
...
*       Fix Me!!! Maybe should protect with a pid flag...
*       -------
	if (xs.gt.(hcal_sh_bottom+hcal_sh_x_offset-2.5) .or.
     >      xs.lt.(hcal_sh_top+hcal_sh_x_offset+2.5) .or.
     >      ys.gt.(hcal_sh_left+hcal_sh_y_offset-2.5) .or.
     >      ys.lt.(hcal_sh_right+hcal_sh_y_offset+2.5) ) then
	   rSTOP_sh = rSTOP_sh + 1
	   stop_where=31.
	   x_stop=xs
	   y_stop=ys
