...
! modifications to vertex.* variables in later calls.
	...
	if (debug_local) write(6,*)'radc_init_ev: entering...'
...
! ... the bt's
	do i=1,2
	  ...
	  if (debug_local) write (6,*) 'radc_init_ev : teff ', main%target%teff(i)
	  if (debug_local) write (6,*) 'radc_init_ev : bt ', bt(i)
	enddo
...
! ... get the hard correction factor. don't care about Ecutoff! Just want dhard here
	...
	if (intcor_mode.eq.0) then
	   if (debug_local) write(6,*)'radc_init_ev: get schwinger...'
	  r = schwinger(Ecutoff,vertex,.true.,dsoft,dhard)
	else
	  if (.not.use_offshell_rad) then
	     if (debug_local) write(6,*)'radc_init_ev: get brem...'
	    r = brem(vertex%Ein,vertex%e%E,Ecutoff,rad_proton_this_ev,dsoft,dhard,
     >		dsoft_prime)
	  else
	     if (debug_local) write(6,*)'radc_init_ev: get bremos...'
...
! ... proton radiation could be off
	...
	if (debug(2)) write(6,*)'basicrad_init_ev: c_ext ',c_ext
	if (debug(2)) write(6,*)'basicrad_init_ev: g_ext ',g_ext
	if (debug(2)) write(6,*)'basicrad_init_ev: bt ',bt
	if (debug(2)) write(6,*)'basicrad_init_ev: c ',c
	if (debug(2)) write(6,*)'basicrad_init_ev: g ',g
...
! ... are we doing deuterium? (i.e. only using a 1D spectral function)
	doing_deuterium = (nrhoPm.eq.1 .and. E_Fermi.lt.1.0) .or.
     > 	   (theory_par%model .eq. 'LAGET_DEUT')
