...
!	  filename = 'worksim/'//base(1:i-1)//'.rzdat'
	  filename = 'worksim/'//base(1:i-1)//'.data'
...
! ******************************************************************
	  ...
	  if (mod(ntried,ninform).eq.1) then
	    write (6,'(1x,a,i10,a,i9,a,e11.4)') 'Generating Event',
...
!	write(6,*) 'dump_all_this_ev',dump_all_this_ev
...
* Track-coordinate and spectrometer common blocks
	...
	namelist /debug_info/ orig, spec, main, recon
...
! ... multiple scattering
	...
	if (mc_smear_addl) then
	  call target_extra_musc(spec%p%sig_musc, dangles_extra)
	else
	  dangles_extra(1)=0.0
	  dangles_extra(2)=0.0
	endif
	main%SP%p%yptar = orig%p%yptar + dangles(1) + dangles_extra(1) + dang_in(1)
	main%SP%p%xptar = orig%p%xptar + dangles(2) + dangles_extra(2) + dang_in(2)*spec%p%cos_th
...
!	  write(6,*) 'z_P_arm , main%target%z,y_P_arm',z_P_arm,main%target%z,y_P_arm
...
!	    write(6,*) 'hadron_arm,z_P_arm , main%target%z,y_P_arm',hadron_arm,z_P_arm,main%target%z,y_P_arm
...
!	  write(6,*) 'z_P_arm , main%target%z,using_tgt_field',z_P_arm,main%target%z,using_tgt_field
          if (using_tgt_field) then
...
!	  main%RECON%p%z = z_P_arm ! added by itabashi
...
!	  write(6,*) '1. recon%p%z, main%target%z',recon%p%z,main%target%z
...
!	 write(6,*) '2. recon%p%z, main%target%z',recon%p%z,main%target%z	
...
!	write(6,*)'rec%p%P; rec%p%E,mh2',recon%p%P,recon%p%E,Mh2
!	write(6,*)'rec%p%P; rec%p%E,mh2',recon%p%P,recon%p%E,Mh2
...
! ... multiple scattering
	...
	if (mc_smear_addl) then
	  call target_extra_musc(spec%e%sig_musc, dangles_extra)
	else
	  dangles_extra(1)=0.0
	  dangles_extra(2)=0.0
	endif
...
!	  main%RECON%e%z = z_E_arm ! changed by itabashi
	  main%FP%e%x = xfp
	  main%FP%e%dx = dxfp
	  main%FP%e%y = yfp
	  main%FP%e%dy = dyfp
...
! Made it!
	...
	subroutine recon_vertex(main, sp, sp_type,
     >                          zv_t, yv_t)
!
! return reconstructed vertex location zv_t, yv_t
!
! WB reconstruct vertex position from reconstruced qunatities
!
	implicit none
	include 'simulate.inc'
	include 'debug_info.inc'
	...
	real*8 dtr
        ...
        real*8 zv_t, yv_t, zv_b, yv_b
        real*8 th_t, ph_t, ytg

        real*8 ubz, uby, utz, uty

! what spectrometer E or P
	character*1 sp_type
...
! spectrometer z-0 position
	ytg = sp%z   ! this is really ytg

! transform to Lab system
!
! thess formulas work for phi = pi/2 and 3pi/2. NOT verified for other values
	if (sp_type .eq.'E') then
	   z_v = -ytg*spec%e%sin_th*sin(spec%e%phi) + spec%e%offset%z
	   y_v =  ytg*spec%e%cos_th + spec%e%offset%y
	elseif( sp_type .eq.'P') then
	   z_v = -ytg*spec%p%sin_th*sin(spec%p%phi) + spec%p%offset%z
	   y_v =  ytg*spec%p%cos_th + spec%p%offset%y
	else
...
! trajectory unitvector
! do not need this
!	utx = sin(th_t)*cos(ph_t)
	uty = sin(th_t)*sin(ph_t)
	utz = cos(th_t)
...
! the intersections should be the same

	zv_b = beta_t*ubz
	yv_b = y_b + beta_t * uby

	zv_t = z_v + alpha_t * utz
	yv_t = y_v + alpha_t * uty
