...
void calc3H(){
  ...
  string  pname="param/Tritium.dat";
  ifstream ifp(pname.c_str());
  string opname="param/3h.dat";
  ...
  gQF->SetTitle("3H Fermi mom distribution ; p_{M} [MeV/c]; probability");
  ...
  while( ifp.eof()==0){
    ...
    else ans=4*3.14/pow(3.14*2.0,3)*ratio[i]*(mom[i])*mom[i]*mom[i]/pow(200.,3.);
    Ans+=ans;
