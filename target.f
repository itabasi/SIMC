...
!       s_Al = s_Al + 0.0028*inch_cm
	     s_Al = s_Al + 0.0311 ! E12-17-003 Hydrogen cell Entrance
!       s_Al = s_Al + 0.0253
!       s_Al = s_Al + 1.*inch_cm ! test by itabashi

	  else if (targ%can .eq. 2) then !pudding can (5 mil Al, for now)
	  s_Al = s_Al + 0.0050*inch_cm
	 else if (targ%can .eq. 3) then	!E12-17-003 Tritium target
	    s_Al = s_Al + 0.0253
	 else if (targ%can .eq. 4) then	!E12-17-003 Hydrogen target
 	    s_Al = s_Al + 0.0311
...
!	    side_path = 0.0503 / abs(sin(theta)) ! E12-17-003  Hydrogen cell 
!	    side_path = 0.0447 / abs(sin(theta)) ! E12-17-003  Tritium cell 
	    if (forward_path.lt.side_path) then
	      s_Al = s_Al + 0.0311*inch_cm / abs(cos(theta)) ! side wall HallA
	    else
	      s_target = side_path
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! <<
	 ...
	 else if (targ%can .eq. 3 .or. targ%can .eq. 4) then !beer can (5 mil Al, for now)
	    side_path = 0.80039*inch_cm / abs(sin(theta)) ! cigar cell in HallA
	    if (forward_path.lt.side_path) then
	      s_Al = s_Al + 0.044 / abs(cos(theta)) ! side wall HallA
	    else
	      s_target = side_path
	      s_Al = s_Al + 0.044 / abs(sin(theta))
	    endif
...
30	continue
	if (hadron_arm.eq.1) then		!proton in HMS
	  ...
	  s_Al = 0.013*inch_cm
!	  s_Al =  0.0311	! E12-17-003 Hydrogen cell Entrance
	  s_air = 15
 	  s_kevlar = 0.*inch_cm
	  s_mylar = 0.010*inch_cm
	  forward_path = (targ%length/2.-zpos) / abs(cos(theta-targ%angle))
...
!	     side_path = 0.0503 / abs(sin(theta)) ! E12-17-003 H side cell
	    if (forward_path.lt.side_path) then
...
!!!!!!!!!!
	 ...
	 else if (targ%can .eq. 3 .or. targ%can .eq. 4 ) then !beer can
	    side_path = 1.325*inch_cm/ abs(sin(theta))
	    if (forward_path.lt.side_path) then
	      s_Al = s_Al + 0.044 / abs(cos(theta))
	    else
	      s_target = side_path
	      s_Al = s_Al + 0.044 / abs(sin(theta))
	    endif
...
! ... compute energy losses
	...
	Eloss=Eloss_target+Eloss_Al+Eloss_air+Eloss_kevlar+Eloss_mylar
!	write(6,*) 'Eloss_target; Eloss_Al; Eloss_air;Eloss_kevlar;Eloss_mylar',Eloss_target,Eloss_Al,Eloss_air,Eloss_kevlar
!	Eloss=0.0		! as a test by itabasi 
	return
	end
...
! ........ wrong for the pudding can targ.  Check it out later.
	...
	else if (targ%can .eq. 3 .or. targ%can .eq. 4) then	!pudding can
! ... for pudding can, max loss occurs at lowest scattering angle, where
! ... the outgoing particle leaves the can at z=0.  Therefore, take minimum
! ... scattering angle, project back from z=0, x=radius to get z_init.
! ... Minimum z_init is -radius.
