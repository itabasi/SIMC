...
!directory set to "//TUPLE"
	...
	if (doing_pion .or. doing_kaon .or. doing_delta) then
	  ...
	  NtupleTag(m) = 't'		! 37
	  m = m+1
	  NtupleTag(m) = 'Trec' ! 38  Trec
	  m = m+1
	  NtupleTag(m) = 'pferx' ! 39  fermi mom x	    
	  m = m+1
	  NtupleTag(m) = 'pfery' ! 40  fermi mom y	    
	  m = m+1
	  NtupleTag(m) = 'pferz' ! 41  fermi mom z	    
	  m = m+1
...
!       added by itabashi
	    ...
	    NtupleTag(m) = 'Rz' ! 58
	    m = m+1	    
	    NtupleTag(m) = 'Ein' ! 59 recon. beam energy
	    m = m+1	    
	    NtupleTag(m) = 'rasx' ! 60 raster x
	    m = m+1	    
	    NtupleTag(m) = 'rasy' ! 61 raster y
	    m = m+1	    
	    NtupleTag(m) = 'Rp_rec' ! 62 recon kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lp_rec' ! 63 recon scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'Rp_orig' ! 64 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lp_orig' ! 65 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lph_orig' ! 66 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Rph_orig' ! 67 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lth_orig' ! 68 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Rth_orig' ! 69 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'pfer' ! 70 fermi  momentum
  	    m = m+1	    
!	    NtupleTag(m) = 'dOmega_e' ! 71 dOmega_e
	    NtupleTag(m) = 'Pm' ! 71 vertex Pm
  	    m = m+1	    
	    NtupleTag(m) = 'Mrec' ! 72 Mrec	    
   	    m = m+1	    
	    NtupleTag(m) = 'efer' ! 73 efer
   	    m = m+1	    
	    NtupleTag(m) = 'nu_gen' ! 74 vertex%nu	    
	    m = m+1
	    NtupleTag(m) = 'nu_rec' ! 75 recon%nu	    
	    m = m+1	    
	    NtupleTag(m) = 'Rp_gen' ! 76 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lp_gen' ! 77 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lph_gen' ! 78 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Rph_gen' ! 79 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'Lth_gen' ! 80 generate kaon momentum
	    m = m+1	    
	    NtupleTag(m) = 'Rth_gen' ! 81 generate scatted electron  momentum
	    m = m+1	    
	    NtupleTag(m) = 'uq_x' ! 82 recon uq_x
	    m = m+1	    
	    NtupleTag(m) = 'uq_y' ! 83 recon uq_y
	    m = m+1	    
	    NtupleTag(m) = 'uq_z' ! 84 recon uq_z	    
	    m = m+1	    
	    NtupleTag(m) = 'uq_x_gen' ! 85 vertex uq_x
	    m = m+1	    
	    NtupleTag(m) = 'uq_y_gen' ! 86 vertex uq_y
	    m = m+1	    
	    NtupleTag(m) = 'uq_z_gen' ! 87 vertex uq_z	    
	    m = m+1	    
	    NtupleTag(m) = 'q_gen' ! 88 vertex q	    
	    m = m+1	    
	    NtupleTag(m) = 'Pmiss' ! 89 vertex Pmiss
	    m = m+1
            NtupleTag(m) = 'mm_L' ! 90  Lambda
	    m = m+1
            NtupleTag(m) = 'mm_S' ! 91  Sigma
            m = m+1
            NtupleTag(m) = 'mm_nnL' ! 92  nnL
            m = m+1
            NtupleTag(m) = 'mm_3HL' ! 93  3HL
!	    NtupleTag(m) = 'up_x' ! 90 recon up_x
!	    m = m+1
!	    NtupleTag(m) = 'up_y' ! 91 recon_up_y
!	    m = m+1
!	    NtupleTag(m) = 'up_z' ! 92 recon_up_z	    
	    !m = m+1
	    !NtupleTag(m) = 'up_x_gen' ! 93 vertex up_x
	    !m = m+1
	    !NtupleTag(m) = 'up_y_gen' ! 94 vertex_up_y
	    !m = m+1
	    !NtupleTag(m) = 'up_z_gen' ! 95 veretex_up_z
	    !m = m+1
	    !NtupleTag(m) = 'central%rad%c(1)' ! 101  Eloss_e okuyama
	    !m = m+1
	    !NtupleTag(m) = 'central%rad%c(2)' ! 102  Eloss_e' okuyama   
	    !m = m+1
	    !NtupleTag(m) = 'central%rad%c(3)' ! 103  Eloss_K+ okuyama  
!	    m = m+1
!	    NtupleTag(m) = 'SurvivalRatio' ! 93  survivalprob okuyama
	    m = m+1
	    NtupleTag(m) = 'PathLength' ! 94  pathlen okuyama   
	    m = m+1
	    NtupleTag(m) = 'BetaK' ! 95  betak okuyama  
	    !!!!!!m = m+1
	    !!!!!!NtupleTag(m) = 'Eloss_e' ! 93  Eloss_e okuyama
	    !!!!!!m = m+1
	    !!!!!!NtupleTag(m) = 'Eloss_ep' ! 94  Eloss_e' okuyama   
	    !!!!!!m = m+1
	    !!!!!!NtupleTag(m) = 'Eloss_k' ! 95  Eloss_K+ okuyama  
	    m = m+1
	    NtupleTag(m) = 'brems_e' ! 96  Eloss_e okuyama
	    m = m+1
	    NtupleTag(m) = 'brems_ep' ! 97  Eloss_e' okuyama   
	    m = m+1
	    NtupleTag(m) = 'brems_k' ! 98  Eloss_K+ okuyama  
	    !m = m+1
	    !NtupleTag(m) = 'pferx' ! 96  pfer x
	    !m = m+1
	    !NtupleTag(m) = 'pfery' ! 97  pfer y
	    !m = m+1
	    !NtupleTag(m) = 'pferz' ! 98  pfer z
	    m = m+1
	    NtupleTag(m) = 'Em_v' ! 99  vetex%Em
	    m = m+1
	    NtupleTag(m) = 'Emiss' ! 100  Emiss	    
!	    m = m+1
!	    NtupleTag(m) = 'Trec' ! 101  Trec
!	    m = m+1
!	    NtupleTag(m) = 'pferx' ! 102  fermi mom x	    
!	    m = m+1
!	    NtupleTag(m) = 'pfery' ! 103  fermi mom y	    
!	    m = m+1
!	    NtupleTag(m) = 'pferz' ! 104  fermi mom z	    
!	    m = m+1
!	    NtupleTag(m) = 'mm_L' ! 101  Lambda
!	    m = m+1
!	    NtupleTag(m) = 'mm_S' ! 102  Sigma
!	    m = m+1
!	    NtupleTag(m) = 'mm_nnL' ! 103  nnL
!            m = m+1
	 endif
	  ...
	  m = m+1
	  NtupleTag(m) = 'tar_zxo'	! 50          ! WB added 
	  m = m+1
