...
c	  ntu(11) = vertex%p%xptar			!mr
	...
	if (doing_pion .or. doing_kaon .or. doing_delta) then
	  ...
	  if(using_tgt_field) then
	     ...
	     ntu(61) = vertex%phi_c	     
	     if (doing_kaon) then
		ntu(62) = ntup%sigcm2 !sigcm - saghai model   JR froget about saghai for now, not working
...
!       sign(pfer/1000., dummy) !p_fermi - GeV/c                          
		...
		ntu(74) = vertex%nu
		ntu(75) = recon%nu
		ntu(76) = vertex%p%p !  generated proton momentum
		ntu(77) = vertex%e%p !  generated electron momentum
		ntu(78) = vertex%e%yptar !  generated electron phi at target
		ntu(79) = vertex%p%yptar !  generated proton phi at target
		ntu(80) = vertex%e%xptar !  generated electron theta at target
		ntu(81) = vertex%p%xptar !  generated proton theta at  target		
		ntu(82) = recon%uq%x ! recon uq_x
		ntu(83) = recon%uq%y ! recon uq_y
		ntu(84) = recon%uq%z ! recon uq_z
		ntu(85) = vertex%uq%x ! vertex uq_x
