...
!	write (6,*) 'xwid, ywid',gen%xwid,gen%ywid ! test by itabashi
...
!	write (6,*) 'fr1,fr2',targ%fr1,targ%fr2 ! test by itabashi

	
	call trip_thru_target (1, main%target%z-targ%zoffset, Ebeam, 0.0d0,
     >		main%target%Eloss(1), main%target%teff(1),Me,1)
	if (debug(2)) then
	   write (6,*) 'gen: vertex :',main%target%x, main%target%y, main%target%z
	   write (6,*) 'gen: Eloss(1), teff(1) :',main%target%Eloss(1), main%target%teff(1)
	endif
...
!	   write (6,*) 'xtar_min ;max; yptar_min;max',gen%e%xptar%min,gen%e%xptar%max,gen%e%yptar%min,gen%e%yptar%max ! test by itabashi
...
!	  write(6,*)'p_xp, p_yp :gen ',vertex%p%xptar,vertex%p%yptar ! itatest
!       write (6,*) 'xtar,ytar' , gen%p%xptar,gen%p%yptar ! test by itabashi!
	endif

!	   write (6,*) 'xtar_min ;max; yptar_min;max',gen%p%xptar%min,gen%p%xptar%max,gen%p%yptar%min,gen%p%yptar%max ! test by itabashi
! -----------------------------------------------------------------------------
...
!	write (6,*) 'xtar,ytar,E' , gen%p%xptar,gen%p%yptar,vertex%p%E ! itabashi
! -----------------------------------------------------------------------------
! Generate Electron Energy (all but hydrogen elastic)
	if (doing_deuterium.or.doing_heavy.or.doing_pion.or.doing_kaon
     >       .or.doing_delta.or.doing_rho.or.doing_semi) then
	  Emin=gen%e%E%min
	  Emax=gen%e%E%max
!	  write(6,*)'Emin :Emax', gen%e%E%min,gen%e%E%max ! ita test
	  if (doing_deuterium .or. (doing_heavy .and. doing_bound) 
     >	     .or. doing_pion .or. doing_kaon .or. doing_delta .or. doing_rho) then
...
!	  write(6,*)'Emin :Emax E, weight', gen%e%E%min,gen%e%E%max,vertex%e%E,main%gen_weight ! ita test	  
...
!	write(6,*)'===== angle ====='
!	write(6,*)'spec_e_theta, spec_e_phi, e_xp, e_yp',spec%e%theta,spec%e%phi,vertex%e%xptar,vertex%e%yptar ! itatest
!	write(6,*)'spec_p_theta, spec_p_phi, p_xp, p_yp',spec%p%theta,spec%p%phi,vertex%p%xptar,vertex%p%yptar ! itatest
!	write(6,*)'e_th, e_ph',vertex%e%theta,vertex%e%phi ! itatest
!	write(6,*)'p_th,p_ph',vertex%p%theta,vertex%p%phi ! itatest
...
!	    write(6,*)'efer,Em,pfer,mp',efer,vertex%Em,pfer,targ%Mtar_struck ! test ita
	 endif
	endif
...
C Call energy loss here - just before sending on to the spectrometers.
	...
	if (debug(2)) then
	   write (6,*) 'gen: after call to generate_rad'
	   write (6,*) 'gen: Eloss(2), teff(2) :',main%target%Eloss(2), main%target%teff(2)
	   write (6,*) 'gen: Eloss(3), teff(3) :',main%target%Eloss(3), main%target%teff(3)
	endif
...
!---------------------------------------------------------------------
	...
	integer picked_solution

	logical success, first
	type(event_main):: main
	type(event)::	vertex
	
	data first/.True./

!	write(6,*)'complete_ev'! itatest	
...
!	write(6,*)'ue_x,ue_y,ue_z',vertex%ue%x,vertex%ue%y,vertex%ue%z ! ita0609
	
!	write(6,*)'e_th,e_ph,p_th,p_ph ',vertex%e%theta,vertex%e%phi,vertex%p%theta,vertex%p%phi 	! itatest
...
!	write(6,*)' uqx,uqy,uqz =',vertex%uq%x,vertex%uq%y,vertex%uq%z			!       test by itabashi
!	write(6,*)'q ,nu,ue_z ',vertex%q,vertex%nu,vertex%ue%z	! itatest
...
! event generation limits.
	  ...
	  picked_solution = 1
	  if (Ehad2.gt.edge%p%E%min .and. Ehad2.lt.edge%p%E%max) then
! WB 2012 of both solutions are within select one or the other randomly
	     if (grnd() .gt. 0.5) then
		vertex%p%E = Ehad2
		picked_solution = 2
	     endif
	  ...
	  if (Ehad2.gt.edge%p%E%min .and. Ehad2.lt.edge%p%E%max .and. ntried.le.5000) then
	     ...
	     write(6,*) 'Actually, picked solution : ', picked_solution, ' randomly'
	  endif
...
!	   write(6,*) 'pfer,Mrec,Em ',pfer,vertex%Mrec,vertex%Em ! itatest
...
!       have big problems.
!	  write(6,*)'a b c Mtar',a,b,c,targ%M ! ita0608
!	  write(6,*)'c, nu, M',c,vertex%nu,targ%M! ita0608
	  if (doing_deutpi.or.doing_hepi.or.doing_deutkaon.or.doing_hekaon.or.
...
!	write(6,*) 'a,b,c, pfer=',a,b,c,pfer ! ita
!	write(6,*) 't=',t/1000000.
...
!     >				vertex.nu+targ.M-(-QB+sqrt(radical))/2./QA
	  ...
	  radical = QB**2 - 4.*QA*QC
!	  write(6,*)'a b c ',a,b,c
!	  write(6,*)'t,QA,QB,QC,rad',t,QA,QB,QC,radical ! ita0608
	  ...
	  Ehad2 = (-QB + sqrt(radical))/2./QA
	  picked_solution = 1
!	  write(6,*) 'Ek ;Ehad2   ',vertex%p%E,Ehad2
	  if (Ehad2.gt.edge%p%E%min .and. Ehad2.lt.edge%p%E%max ) then
	     if (grnd().gt.0.5) then
	       vertex%p%E = Ehad2
	       picked_solution = 2
...
!       write(6,*) 'Ek ,QA,QB,rad   ',vertex%p%E,QB,QA,radical ! ita0608
...
!              write(6,*) ' e1, e2=',vertex%p%E,Ehad2
	 ...
	 vertex%p%P = sqrt(vertex%p%E**2 - Mh2)
	 vertex%p%delta = (vertex%p%P - spec%p%P)*100./spec%p%P
!	write(6,*) 'Pk,Ek=',vertex%p%P,vertex%p%E ! 

!	 write(6,*) 'Erec, Mrec,pfer',E_rec,targ%Mrec_struck,pfer	! ita0608
!	 write(6,*)'Ek,a,b,c ',vertex%p%E,a,b,c ! ita0616
! itatest 0609
!	  write(6,*)'nu,Q2,q,Mh2, Ein,Ee_ ',vertex%nu,vertex%Q2,vertex%q,Mh2,vertex%Ein,vertex%e%E
!	  write(6,*)'uqx,uqy,uqz,a',vertex%uq%x,vertex%uq%y,vertex%uq%z,a ! ita0609
!	  write(6,*)'t,QA,QB,QC,rad',t,QA,QB,QC,radical
!	  write(6,*) 'p,e=',vertex%p%P,vertex%p%E
!	  write(6,*)'-- end ----'
!vertex%e%E.gt.vertex%Ein
...
C       DJG Calculate the "Collins" (phi_pq+phi_targ) and "Sivers"(phi_pq-phi_targ) angles
...
! WB carefule here the missing momentum is NOT the recoil momentum
! it is defined via P_f = q + Pm => Pm = P_f - q
! 
! the recoil momentum is normally defined via:
!
! P_f + P_r = q => P_r = q - P_f = - P_m
...
! WB I would claculate the 3 unit vectors and then calculate the projections
...
! A>2 case is some approximation for 3He (DJG).
	...
	ntup%krel = krel
!	Write(6,*)'ver_nu ver_Pm,MM',vertex%nu,vertex%Pm,MM
	  ! test by itabashi 0616
...
! store for later use
	main%jacobian_corr = 1. / r**3		 !1/cos**3(theta-theta0)

C DJG Since we generate rho's in 4pi (in spherical angles) we don't need no
C DJG stinkin' Jacobian!

C WB why is this not needed for deuterium since there is also a selecton of proton directions ?
C	if (doing_heavy .or. doing_pion .or. doing_kaon .or. 
...
C WB these calls are necessary otherwise the teff's in radc_init_ev are wrong
...
!	write(6,*)'complete_ev-succeed'! itatest
	
	success = .true.
	if(debug(2)) write(6,*)'comp_ev: at end...'
!	write(6,*)'comp_ev: at end...' ! itatest
	return
	end
...
!	write(6,*)'complete_recon_ev'! itatest
!-----------------------------------------------------------------------
...
!
! Theta Element of /event/ NOT computed here is sigcc (for (e,e'p)).
!
!-----------------------------------------------------------------------
...
! unit vector
	uPmx = recon%Pmx/recon%Pm
	uPmy = recon%Pmy/recon%Pm
	uPmz = recon%Pmz/recon%Pm
! WB calculate angle theta_rq (recoil angle)

	cthrq = uPmx*recon%uq%x + uPmy*recon%uq%y + uPmz*recon%uq%z
	if (abs(cthrq) .gt. 1.d0) then
	   cthrq = dsign(1.d0, cthrq)
	endif
! the - sign is necessary on order to get the recoil angle
	recon%theta_rq=acos(-cthrq)
...
!STILL NEED SIGN FOR PmPer!!!!!!
	...
	if (doing_pion .or. doing_kaon .or. doing_delta .or. doing_rho .or. doing_semi) then
	  ...
	  !write(6,*) 'reconEm,targMtar_struck,reconpE',	  recon%Em,targ%Mtar_struck,recon%p%E
     ...
     >      + 2*(recon%nu*recon%p%E - recon%p%P*recon%q*cos(recon%theta_pq))
	  ...
	  ntup%mmA = mmA
	  ntup%t = t
!	  Write(6,*)'rec_nu rec_Pm',recon%nu,recon%Pm
	  ! test by itabashi 0616
!	  if(ntup%mmA.le.2999.) then
!	     write(6,*)'mass ,massA',ntup%mm,ntup%mmA
!	  endif
	endif
...
!       This main%weight is the one written in the ntuple
     ...
     >                   main%jacobian*main%jacobian_corr*
     >                   main%gen_weight*
     >                   main%sigcc*
     >                   main%coul_corr
!	   write(6,*)'weight 2',main%weight ! ita
	endif
...
!	write(6,*)'weight ',main%weight  ! test by itabashi
	if (debug(5))write(6,*) 
     > 'SF_weight, jac., gen_weight, sigcc, tgt_weight = ',
     >        main%SF_weight,
     >	      main%jacobian,
     >        main%gen_weight,
     >        main%sigcc,
...
!	write(6,*) 'theta0,phi0,sinth,sinph,costh,cosph',theta0,phi0,sinth,sinph,costh,cosph !ita0609	
	if (dx.ne.0.0) then
	  phi = atan( (dy*costh + sinth*sinph) / dx )	!gives -90 to 90 dego.
...
!	write(6,*) 'dx,dy, r, tmp, theta, phi',dx,dy,r,tmp,theta,phi !ita0609
