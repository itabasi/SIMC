...
!       RCT 8/4/2016 This corresponds to the 3-body break up
	if (.not.doing_bound) then ! doing in He file
!	   write(6,*)' Em ,Emval ,numEm',Em,Emval(numEm),numEm ! itatest 0725
...
!	        write(6,*) 'sf1,sval,Em1,Pm,iEm,iPm',sf1,sfval(iEm,iPm),Pmval(iPm),Em1,iEm,iPm
!	        write(6,*) 'sval(i,j), sval(i+1,j)',sfval(iEm,iPm),sfval(iEm+1,iPm)
	      endif
	    enddo
! << -------------------------------------------------------------- >>!!
	  endif
...
! if He or 3H run was used, SF function is given logsf 
...
! y() is SF value
! Pm  is X(A-1) momentum .data parameters
